...
#' @param W.hat_test predicted propensity score by regression_forest for test set
...
SUBGROUP_PIPELINE<- function(X, 
  ...
  DATA4grpmodel_iCF_tr     <- DATA4grpmodel(vote_D5_subgroup, vote_D4_subgroup, vote_D3_subgroup, vote_D2_subgroup, Train_ID)
  
  #according to Algorithm, we get G_D from training set, then buidl model for Y* in testing set!!!
  DATA4grpmodel_iCF_te     <- DATA4grpmodel(vote_D5_subgroup, vote_D4_subgroup, vote_D3_subgroup, vote_D2_subgroup, Test_ID )
  
  #extracting training or testing set with g_D, and g_D definition (defined by spliting variable and splitting values!)
  Train_ID_SG_iCF       <- DATA4grpmodel_iCF_tr$Dat_ID_SG
  Test_ID_SG_iCF        <- DATA4grpmodel_iCF_te$Dat_ID_SG
  ...
  formula_g2   <<- DATA4grpmodel_iCF_tr$formula_g2
  formula_g3   <<- DATA4grpmodel_iCF_tr$formula_g3
  formula_g4   <<- DATA4grpmodel_iCF_tr$formula_g4
  formula_g5   <<- DATA4grpmodel_iCF_tr$formula_g5
  
#Function that obtain models from subgroup decisions at different depth in the training set
  Deci_Final_iCF.tran.tr       <- CF_GROUP_DECISION(HTE_P_cf.raw, Train_ID_SG_iCF, "iCF", "transform", P_threshold, W.hat)
