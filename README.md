...
<img src = images/FIGURE1_all_Panels_AB.jpg width=1000>
...
packages_to_install <- c("MASS", "grf", "tidyverse", "rlang", "rlist", "plyr", "caret", "caTools", "listdtr", "randomForest", "ggplot2",
                         "ggridges", "data.table", "grid", "cobalt", "broom", "optmatch", "rstatix", "ggpubr", "DMwR", "knitr", "DiagrammeR")
...
```
Make a dataset in the "data.frame" format to be run by iCF: the 1st column is outcome **Y**, 2nd column is treatment **W**, and remaining columns features **X**.
```{}
...
```{}
D2_MLS=MinLeafSizeTune(Train, denominator=25, treeNo = 200, iterationNo=100, split_val_round_posi=0, "D2", "steelblue1")
D2_MLS$depth_mean
D2_MLS$depth_gg
```
...
```{}
D3_MLS=MinLeafSizeTune(Train, denominator=45, treeNo = 200, iterationNo=100, split_val_round_posi=0, "D3", "steelblue1")
D3_MLS$depth_mean
D3_MLS$depth_gg
```
...
```{}
D4_MLS=MinLeafSizeTune(Train,denominator=65, treeNo = 200, iterationNo=100, split_val_round_posi=0, "D4", "steelblue1")
D4_MLS$depth_mean
D4_MLS$depth_gg
```
...
```{}
D5_MLS=MinLeafSizeTune(Train, denominator=85, treeNo = 200, iterationNo=100, split_val_round_posi=0, "D5", "steelblue1")
D5_MLS$depth_mean
D5_MLS$depth_gg
```
...
Make a dataset in the "data.frame" format to be run by iCF: the 1st column is treatment **W**, 2nd column is outcome **Y**, and remaining columns features **X**. And the 1st and 2nd columns should be named following this pattern, i.e., 'W' for treatment and 'Y' for outcome.
...
```
**Empirical Studies Implementing iCF in real-world data:**

Lu W, Tang H, Wang J, Han J, Li Y, Wang X, Hou J. Heterogeneous effects of preoperative beta blockers on 30-day mortality after coronary artery bypass surgery. Ann Med. 2026 Dec;58(1):2719256. [DOI: 10.1080/07853890.2026.2719256](https://doi.org/10.1080/07853890.2026.2719256). 
...
Wang T, Wang J, Kinlaw AC, Wyss R, Pate V, Gou Z, Buse JB, Keet CA, Kosorok MR, Stürmer T. Glucagon-like Peptide 1 Receptor Agonists in Asthma Exacerbations: an Application of High-dimensional Iterative Causal Forest to Identify Subgroups. Pharmacoepidemiol Drug Saf. 2025;34:e70192. [DOI: 10.1002/pds.70192](https://doi.org/10.1002/pds.70192)
